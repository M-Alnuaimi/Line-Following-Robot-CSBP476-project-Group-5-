...
void loop() 
{
    ...
    BlackLineTracking(); //run main program
    if(M_val==0 && R_val==1 && L_val==1){
      check = 1;
    }
  if(check==1){
    WhiteLineTracking(); 
    if(M_val==1 && R_val==0 && L_val==0){
      check = 2;
    }
  }
  
  if(check==2){
    if(M_val==0 && R_val==0 && L_val==0){
      left();
  }
    BlackLineTracking(); //run main program
    if(M_val==1 && R_val==1 && L_val==1){ //Reached End Line
      Stop();
      check = 3;
    }
...
void BlackLineTracking()
{
  ...
  else if(M_val==0){ //No black lines detected in the middle
    if (L_val == 1 && R_val == 0) { //If a black line is detected on the left, but not on the right, turn left
      left();
    }
    else if (L_val == 0 && R_val == 1) { //Otherwise, if a black line is detected on the right and not on the left, turn right
  ...
  else if(M_val==0 && R_val==0 && L_val==0){
    right();
  }
}
...
void WhiteLineTracking()
{
  ...
  if(M_val == 0){//if the state of middle one is 1, which means detecting White line

     if (L_val == 0 && R_val == 1) { //If a White line is detected on the left, but not on the right, turn left
        left();
    }
     else if (L_val == 1 && R_val == 0) { //Otherwise, if a White line is detected on the right and not on the left, turn right
      right();
    }
     else { //Otherwise,forward
      front();
  ...
  else if(M_val==1){ //if black line detected in the middle
    if (L_val == 0 && R_val == 1) { //If a White line is detected on the left, but not on the right, turn left
      left();
    }
    else if (L_val == 1 && R_val == 0) { //Otherwise, if a White line is detected on the right and not on the left, turn right
  ...
  else if(M_val==1 && R_val==1 && L_val==1){ //if entered the black area, return back
    back();
  }
}
